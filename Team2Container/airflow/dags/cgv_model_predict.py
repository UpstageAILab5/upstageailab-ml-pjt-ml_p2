...
def prepare_data(index:int, **context):
    ...
    code_list = []
    code_list += list(cgv_movies.keys())
...
def slack_notification(**context):
    ...
    message = f"""
* CGV 영화 데이터 업로드 정보 *
- {cgv_movies[movie_0_code]}: 업로드 성공
- {cgv_movies[movie_1_code]}: 업로드 성공
- {cgv_movies[movie_2_code]}: 업로드 성공
- {cgv_movies[movie_3_code]}: 업로드 성공
    """
