...
S3_BUCKET_PATH = "cinema/cgv/review/"
...
def prepare_data(**context):
    cgv_movies = {"88381":"moana2", "88076":"wicked", "89075":"1승", "88459":"gladiator2"}
    cgv_movie_codes = list(cgv_movies.keys())

    data = context['ti']
    data.xcom_push(key='movies', value=cgv_movies)
    data.xcom_push(key='movie_codes', value=cgv_movie_codes)
...
def slack_notification(**context):
    ...
    cgv_movies = data.xcom_pull(task_ids='prepare_task', key='movies')
    
    message = f"""
* CGV 영화 리뷰 크롤링 업로드 정보 *
- {cgv_movies[movie_0_code]}: 업로드 {movie_0_result}
- {cgv_movies[movie_1_code]}: 업로드 {movie_1_result}
- {cgv_movies[movie_2_code]}: 업로드 {movie_2_result}
- {cgv_movies[movie_3_code]}: 업로드 {movie_3_result}
    """
